...
class Game extends React.Component<GameProps> {
  ...
  createGameLoop = () => {
    return accurateInterval(
      () => {
        ...
        for (let tile of this.state.tiles) {
          if (this.sliders.includes(tile.type)) {
            if (this.state.currHexNote === tile.hexNote) {
              tile.ref.current.stopSlider();
            }
          }
          if (
  ...
  showTile(tile: Tiles, j: number) {
    return (
      this.compareLetters[j].includes(tile.type) &&
      this.state.gameSettings.devStartingHexNote <= tile.hexNote &&
      this.state.currHexNote > tile.hexNote &&
      this.state.currHexNote <=
        tile.hexNote + 21 + (tile.length ? tile.length * 2 : 2)
    );
  }
                                ...
                                <>
                                  <img
                                    src={(() => {
                                      switch (tile.type) {
                                        case "U":
                                          return "assets/pictures/arrows/up.svg";
                                        case "L":
                                          return "assets/pictures/arrows/left.svg";
                                        case "R":
                                          return "assets/pictures/arrows/right.svg";
                                        case "D":
                                          return "assets/pictures/arrows/down.svg";
                                      }
                                    })()}
                                    alt={(() => {
                                      switch (tile.type) {
                                    ...
                                    style={{
                                      animation: this.showTile(tile, j)
                                        ? `tile_animation ${
